...
import phylodynamics.util.SEIRStateDouble;
import phylodynamics.util.SEIRStateMoments;
 ...
 * @author tvaughan
 */
public class HybridTauleapSEIR implements SEIR_simulator {
...
//		this.random = random;
    ...
    public void setRates(double expose, double infect, Double[] recover, double loseImmunity, double alpha) {
        ...
        if (loseImmunity>0.) throw new UnsupportedOperationException();  //todo: implement loseImmunity (SIRS)
	...
	public boolean step(double dt, int index) {
		...
		while (true) {
			...
			if (state.S < a_infect*dtCrit + alpha*Math.sqrt(a_infect*dtCrit)) {
				...
				double thisdt = -Math.log(Randomizer.nextDouble())/a_infect;
				if (thisdt < dtCrit) {
					dtCrit = thisdt;
			...
			if (state.I < a_recover*dtCrit + alpha*Math.sqrt(a_recover*dtCrit)) {
				...
				double thisdt = -Math.log(Randomizer.nextDouble())/a_recover;
				if (thisdt < dtCrit) {
					dtCrit = thisdt;
	...
	public boolean step_exposed(double dt, int index) {
		...
		while (true) {
			...
			if (state.S < a_expose*dtCrit + alpha*Math.sqrt(a_expose*dtCrit)) {
				...
				double thisdt = -Math.log(Randomizer.nextDouble())/a_expose;
				if (thisdt < dtCrit) {
					dtCrit = thisdt;
					critReactionToFire = 1;
				}
...
//			System.exit(1);
     ...
     * @param criticalTrajectories list of critical trajectories
	 *
	...
	public List<SEIRState> genTrajectory(double T, int Nt, int Nsamples, List<Integer> criticalTrajectories, double[] times) {
		...
		for (int tidx=1; tidx<Nt; tidx++) {
			...
			if (useExposed)
				crit = step_exposed(dt, 0); // todo: index
			else
				crit = step(dt, 0); // todo: index
...
//							criticalTrajectories.get(tidx/stepsPerSample)+1);
    ...
    // slightly modified method for BEAST Operator
		...
		for (int tidx=1; tidx<Nt; tidx++) {
			...
			if (useExposed)
				step_exposed(dt, 0); // todo: index
			else
				step(dt, 0); // todo: index
...
// ----------------------------------------------------------------------------------------------------------------------------------------------------------------
        ...
        List<SEIRState> traj = hybridTauleapSEIR.genTrajectory(T, Nt, Nsamples, null, null);
		...
		for (int i=0; i<Ntraj; i++) {
			...
			trajectoryList.add(hybridTauleapSEIR.genTrajectory(T, Nt, Nsamples, criticalTrajectories, null));
		}
