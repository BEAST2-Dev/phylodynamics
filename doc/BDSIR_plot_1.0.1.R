...
loglist = read.table("logs.list", as.is=TRUE, header=F) #list of logfilename(s)

finalSampleTime = 0 # time of most recent sample, please edit!
...
# calculations for plotting trajectories:	
	...
	# plot SIR trajs
	plot(1, main='SIR plot', xlim=c(t[1], finalSampleTime), ylim = c(0, N), col="white", ylab='', xlab="(a)")
	...
	plot(1, main="Prevalence & Incidence", xlim=c(t[1], finalSampleTime), ylim = c(0, max(inc,I)), col="white", ylab='', xlab="(b)")
...
# calculations for plotting R over time:	
	...
	#plot median R
	plot(t,main="Effective reproduction number over time", R_perInterval[i,], ylim=c(0, max(R_perInterval_HPD,na.rm=T)), type='l', ylab='R',  xlab="(c)")		
